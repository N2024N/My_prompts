...
try:
    ...
    print("SUCCESS - *** Notion连接成功！权限问题已解决 ***")
    ...
    if query_result and hasattr(query_result, "results"):
        ...
        if count > 0:
            print(f"First record ID: {query_result.results[0].get('id', 'N/A')}")
    else:
        print("INFO - 查询成功但无结果（可能是空数据库）")
        
except Exception as e:
    print(f"ERROR - Notion访问失败: {type(e).__name__}: {e}")
    error_msg = str(e)
    if "Could not find database" in error_msg:
        print("INFO - 错误说明: 集成能看到数据库但无访问权限")
        print("INFO - 解决方案: 检查集成的工作区成员权限")
    elif "API token is invalid" in error_msg:
        print("INFO - 错误说明: API密钥无效")
    elif "rate limit" in error_msg.lower():
        print("INFO - 错误说明: API速率限制")
    else:
        print("INFO - 未知错误，需要进一步排查")